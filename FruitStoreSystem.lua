...
-- Load Fruit Store System from external file with game load wait
local fruitUI = nil

-- Wait for game to load before creating fruit UI
task.spawn(function()
    -- Wait for essential game services to load
    local ReplicatedStorage = game:GetService("ReplicatedStorage")
    local Players = game:GetService("Players")
    ...
    if not Data then
        warn("Data folder not found after 10 seconds")
        return
    end
    
    -- Wait for Config folder in ReplicatedStorage
    local Config = ReplicatedStorage:WaitForChild("Config", 10)
    if not Config then
        warn("Config folder not found after 10 seconds")
        return
    end
    
    -- Wait for ResPetFood config
    local ResPetFood = Config:WaitForChild("ResPetFood", 10)
    if not ResPetFood then
        warn("ResPetFood config not found after 10 seconds")
        return
    end
    
    -- Now load the fruit system
    pcall(function()
        ...
        if fruitSystem then
            ...
            print("✅ Fruit Store System loaded successfully!")
        else
            Tabs.FruitTab:Paragraph({
                Title = "🍎 Fruit Store System",
                Desc = "Fruit store system could not be loaded. Please check your internet connection.",
                Image = "apple",
                ImageSize = 18,
            })
...
Tabs.FruitTab:Paragraph({
    Title = "🍎 Fruit Store System",
    Desc = "Loading fruit store system... Please wait for the game to fully load.",
    Image = "apple",
    ImageSize = 18,
})
...
task.spawn(function()
    task.wait(2) -- Wait longer for UI and fruit system to load
