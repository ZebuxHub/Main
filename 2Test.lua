...
local function updateAvailableEggs()
    ...
    -- ALWAYS add regular eggs first since they can use regular farm tiles
    if availableRegularTiles > 0 then
        -- Regular farms available, add regular eggs FIRST
        for _, egg in ipairs(regularEggs) do
            table.insert(prioritizedEggs, egg)
        end
        print("🥚 Added", #regularEggs, "regular eggs to priority list (regular tiles:", availableRegularTiles, ")")
    else
        print("🥚 Skipping regular eggs - no regular farm tiles available")
    end
    
    if availableWaterTiles > 0 then
        -- Water farms available, add ocean eggs AFTER regular eggs
        for _, egg in ipairs(oceanEggs) do
            table.insert(prioritizedEggs, egg)
        end
        print("🥚 Added", #oceanEggs, "ocean eggs to priority list (water tiles:", availableWaterTiles, ")")
    else
    ...
    for i, egg in ipairs(availableEggs) do
        ...
        if isOceanEgg(egg.type) then
            ...
            for j, part in ipairs(waterFarmParts) do
                local isOccupied = isFarmTileOccupied(part, 3) -- Reduced from 6 to 3 studs
                if not isOccupied then
                    availableWaterTiles = availableWaterTiles + 1
