...
-- Services-- Build A Zoo: Auto Buy Egg using WindUI
...
local function runAutoPlace()
    while autoPlaceEnabled do
        local ok, err = pcall(function()
            ...
            placeStatusData.lastAction = "Preparing to place at tile"
            updatePlaceStatusParagraph()

            -- Ensure proximity: move near tile if far, then wait for BlockInd (server likely validates range)
            local hrpPos = getPlayerRootPosition() or Vector3.new()
            if (hrpPos - pos).Magnitude > 18 then
                pcall(function()
                    local char = Players.LocalPlayer.Character
                    if char then
                        local hum = char:FindFirstChildOfClass("Humanoid")
                        if hum then hum:MoveTo(pos + Vector3.new(0, 2, 0)) hum.MoveToFinished:Wait(2.0) end
                    end
                end)
            end

            -- Try to use BlockInd when available
            local useCF = CFrame.new(pos)
            do
            ...
            task.wait(0.05)
            -- Fire using chosen CFrame
            local args = {
                "Place",
                {
                    DST = vector.create(useCF.Position.X, useCF.Position.Y, useCF.Position.Z),
                    ID = petUID
                }
            }
