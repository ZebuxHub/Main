...
-- Services
local Players -- Build A Zoo: Auto Buy Egg using WindUI
...
local function chooseOwnedFruitName()
    -- Pick the selected fruit with highest count; if none selected, consider all fruits
    local candidates = {}
    if selectedFeedFruitSet and next(selectedFeedFruitSet) ~= nil then
        for n, _ in pairs(selectedFeedFruitSet) do table.insert(candidates, n) end
    else
        candidates = getAllFruitNames()
    end
    local bestName, bestCount = nil, -1
    for _, name in ipairs(candidates) do
        local cnt = getAssetCountForFruit(name)
        if cnt and cnt > bestCount then
            bestName, bestCount = name, cnt
        end
    end
    if bestCount and bestCount > 0 then return bestName end
    return nil
end

local function setDeploySlot(slotIndex, value)
    local deploy = getDeployContainer()
    if not deploy then return false end
    local key = "S" .. tostring(slotIndex)
    local ok = pcall(function() deploy:SetAttribute(key, value) end)
...
local function tryFeedFromEntry(petEntry)
    ...
    -- Equip chosen fruit to S3 and press '3' to hold
    local chosenFruit = chooseOwnedFruitName()
    if not chosenFruit then
        feedStatus.last = "No chosen fruit available"
        updateFeedStatus()
        return false
    end
    local equipName = tostring(chosenFruit)
    if not equipName:match("^PetFood_") then equipName = "PetFood_" .. equipName end
    setDeploySlot(3, equipName)
    pcall(function()
