...
-- Set default keybind to toggle the UI
...
Tabs.MiscTab = Tabs.MainSection:Tab({ Title = "Quest", Icon = "settings"})
Tabs.PerfTab = Tabs.MainSection:Tab({ Title = "Performance [Premium]", Icon = "activity", Locked = true})
Tabs.TrashTab = Tabs.MainSection:Tab({ Title = "Auto Trade [Premium]", Icon = "trash-2", Locked = true})
Tabs.WebhookTab = Tabs.MainSection:Tab({ Title = "Webhook [Premium]", Icon = "webhook", Locked = true})
...
-- Forward declare to avoid nil upvalue during function creation
local parseConveyorIndexFromId
...
function parseConveyorIndexFromId(idStr)
...
-- ============ Save Settings Tab ============
Tabs.SaveTab:Section({ Title = "⌨️ UI Controls", Icon = "keyboard" })

-- Keybind Configuration
Tabs.SaveTab:Keybind({
    Title = "UI Toggle Keybind",
    Desc = "Change the key to open/close the UI",
    Value = "G",
    Callback = function(key)
        Window:SetToggleKey(Enum.KeyCode[key])
        WindUI:Notify({
            Title = "⌨️ Keybind Updated",
            Content = "Press '" .. key .. "' to open/close the UI",
            Duration = 3,
            Icon = "keyboard"
        })
    end
...
task.spawn(function()
    ...
    -- Load Misc System (Lottery, Auto Potion, Auto Like)
    pcall(function()
        local success, result = pcall(function()
            return loadstring(game:HttpGet("https://raw.githubusercontent.com/ZebuxHub/Main/refs/heads/main/MiscSystem.lua"))()
        end)
        local miscModule = nil
        if success then miscModule = result end
        if not miscModule then
            -- Fallback to local file if GitHub unavailable
            pcall(function()
                miscModule = require(script.Parent and script.Parent:FindFirstChild("MiscSystem") or nil)
            end)
        end
...
-- Load simple UI Toggle button (GitHub first, local fallback)
pcall(function()
    local ok, mod = pcall(function()
        return loadstring(game:HttpGet("https://raw.githubusercontent.com/ZebuxHub/Main/refs/heads/main/UIToggleButton.lua"))()
    end)
    if ok and mod and mod.Init then
        mod.Init({ WindUI = WindUI, Window = Window })
    end
end)
