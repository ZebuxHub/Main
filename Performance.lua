...
Tabs.UnlockTab = Tabs.MainSection:Tab({ Title = "🔓 | Unlock Tiles"})
Tabs.DeleteTab = Tabs.MainSection:Tab({ Title = "🗑️ | Auto Delete"})
...
-- New Performance Boost (safer and more effective)
local perfState = {
    applied = false,
    lighting = {},
    terrain = {},
...
local function safelySet(instance, prop, value)
    pcall(function()
        instance[prop] = value
    end)
end
...
local function applyPerformanceBoost()
    if perfState.applied then
        WindUI:Notify({ Title = "🚀 Performance", Content = "Already applied", Duration = 2 })
        return
    end
    ...
    -- Save originals
    perfState.lighting.GlobalShadows = Lighting and Lighting.GlobalShadows
    perfState.lighting.EnvironmentDiffuseScale = Lighting and Lighting.EnvironmentDiffuseScale
    perfState.lighting.EnvironmentSpecularScale = Lighting and Lighting.EnvironmentSpecularScale

    if Terrain then
        perfState.terrain.Decoration = Terrain.Decoration
        perfState.terrain.WaterWaveSize = Terrain.WaterWaveSize
        perfState.terrain.WaterWaveSpeed = Terrain.WaterWaveSpeed
        perfState.terrain.WaterTransparency = Terrain.WaterTransparency
        perfState.terrain.WaterReflectance = Terrain.WaterReflectance
    end
    perfState.cameraFOV = Camera and Camera.FieldOfView
    ...
    if Lighting then
        ...
        for _, child in ipairs(Lighting:GetChildren()) do
            if child:IsA("PostEffect") then
                perfState.postEffects[child] = child.Enabled
                safelySet(child, "Enabled", false)
            elseif child:IsA("Atmosphere") then
                perfState.postEffects[child] = child.Density
    ...
    if Camera then
        safelySet(Camera, "FieldOfView", math.clamp((perfState.cameraFOV or 70) - 10, 40, 120))
    end

    -- Disable heavy effects across workspace (particles, trails, beams)
    task.spawn(function()
    ...
    -- Attempt to reduce other costs
    pcall(function()
        workspace.StreamingEnabled = true
    end)

    perfState.applied = true
    WindUI:Notify({ Title = "🚀 Performance", Content = "Performance boost applied", Duration = 3 })
end

local function restorePerformance()
    if not perfState.applied then
        WindUI:Notify({ Title = "🚀 Performance", Content = "Nothing to restore", Duration = 2 })
        return
    end
    local Lighting = game:GetService("Lighting")
    local Terrain = workspace:FindFirstChildOfClass("Terrain")
    ...
    if Lighting then
        if perfState.lighting.GlobalShadows ~= nil then safelySet(Lighting, "GlobalShadows", perfState.lighting.GlobalShadows) end
        if perfState.lighting.EnvironmentDiffuseScale ~= nil then safelySet(Lighting, "EnvironmentDiffuseScale", perfState.lighting.EnvironmentDiffuseScale) end
        if perfState.lighting.EnvironmentSpecularScale ~= nil then safelySet(Lighting, "EnvironmentSpecularScale", perfState.lighting.EnvironmentSpecularScale) end
        for inst, prev in pairs(perfState.postEffects) do
            if inst and inst.Parent then
                if inst:IsA("PostEffect") then
                    safelySet(inst, "Enabled", prev and true or false)
                elseif inst:IsA("Atmosphere") then
                    safelySet(inst, "Density", tonumber(prev) or 0)
                end
            end
        end
    end
    if Terrain then
        if perfState.terrain.Decoration ~= nil then safelySet(Terrain, "Decoration", perfState.terrain.Decoration) end
        if perfState.terrain.WaterWaveSize ~= nil then safelySet(Terrain, "WaterWaveSize", perfState.terrain.WaterWaveSize) end
        if perfState.terrain.WaterWaveSpeed ~= nil then safelySet(Terrain, "WaterWaveSpeed", perfState.terrain.WaterWaveSpeed) end
        if perfState.terrain.WaterTransparency ~= nil then safelySet(Terrain, "WaterTransparency", perfState.terrain.WaterTransparency) end
        if perfState.terrain.WaterReflectance ~= nil then safelySet(Terrain, "WaterReflectance", perfState.terrain.WaterReflectance) end
    end
    if Camera and perfState.cameraFOV ~= nil then
        safelySet(Camera, "FieldOfView", perfState.cameraFOV)
    end

    perfState.applied = false
    perfState.postEffects = {}
    WindUI:Notify({ Title = "🚀 Performance", Content = "Restored graphics", Duration = 3 })
end

Tabs.PerfTab:Button({
    Title = "🚀 Apply Performance Boost",
    Desc = "Disable heavy effects, shadows, water, and lower detail for FPS",
    Callback = function()
...
Tabs.PerfTab:Button({
    Title = "↩️ Restore Graphics",
    Desc = "Re-enable visual settings changed by the boost",
    Callback = function()
        restorePerformance()
    end
})

-- Ultra Clay Mode (extreme low-fidelity visuals)
local clayState = {
    applied = false,
    lighting = {},
    streaming = {},
    quality = nil,
    parts = {}, -- [Instance] = {Material, CastShadow, Reflectance, MaterialVariant, RenderFidelity?}
    decals = {}, -- {inst, prevTransparency}
    textures = {}, -- {inst, prevTransparency}
    surfaceApps = {}, -- {inst, parent}
}

local function applyClayMode()
    if clayState.applied then
        WindUI:Notify({ Title = "🧱 Clay Mode", Content = "Already applied", Duration = 2 })
        return
    end
    ...
    -- Save and apply lighting for ultra-low fidelity
    clayState.lighting = {
        Ambient = Lighting and Lighting.Ambient,
        OutdoorAmbient = Lighting and Lighting.OutdoorAmbient,
        Brightness = Lighting and Lighting.Brightness,
        FogStart = Lighting and Lighting.FogStart,
        FogEnd = Lighting and Lighting.FogEnd,
        Technology = Lighting and Lighting.Technology,
        ColorShift_Top = Lighting and Lighting.ColorShift_Top,
        ColorShift_Bottom = Lighting and Lighting.ColorShift_Bottom,
    }
    ...
    -- Streaming settings (smaller radii)
    clayState.streaming = {
    ...
    -- Quality level to minimum (best effort)
    pcall(function()
        clayState.quality = settings().Rendering.QualityLevel
    ...
    -- Massive visual flattening across workspace (async)
    task.spawn(function()
        for _, inst in ipairs(workspace:GetDescendants()) do
            if inst:IsA("BasePart") then
                ...
                clayState.parts[inst] = data
                safelySet(inst, "Material", Enum.Material.Plastic)
                safelySet(inst, "CastShadow", false)
                safelySet(inst, "Reflectance", 0)
                pcall(function()
                    inst.MaterialVariant = nil
                end)
                if inst:IsA("MeshPart") then
                    ...
                    if prev ~= nil then
                        clayState.parts[inst].RenderFidelity = prev
            end
        end
            elseif inst:IsA("Decal") then
                table.insert(clayState.decals, {inst = inst, prev = inst.Transparency})
                safelySet(inst, "Transparency", 1)
            elseif inst:IsA("Texture") then
                table.insert(clayState.textures, {inst = inst, prev = inst.Transparency})
                safelySet(inst, "Transparency", 1)
            elseif inst:IsA("SurfaceAppearance") then
                table.insert(clayState.surfaceApps, {inst = inst, parent = inst.Parent})
                pcall(function()
                    inst.Parent = nil
                end)
            end
    ...
    clayState.applied = true
    WindUI:Notify({ Title = "🧱 Clay Mode", Content = "Ultra low-fidelity visuals applied", Duration = 3 })
end

local function restoreClayMode()
    if not clayState.applied then
        WindUI:Notify({ Title = "🧱 Clay Mode", Content = "Nothing to restore", Duration = 2 })
        return
    end
    ...
    -- Restore lighting
    if Lighting then
        for k, v in pairs(clayState.lighting) do
            pcall(function()
                Lighting[k] = v
            end)
        end
    ...
    -- Restore streaming
    pcall(function()
        if clayState.streaming.enabled ~= nil then workspace.StreamingEnabled = clayState.streaming.enabled end
        if clayState.streaming.minRadius ~= nil then workspace.StreamingMinRadius = clayState.streaming.minRadius end
        if clayState.streaming.targetRadius ~= nil then workspace.StreamingTargetRadius = clayState.streaming.targetRadius end
    end)

    -- Restore quality
    pcall(function()
        if clayState.quality ~= nil then
            settings().Rendering.QualityLevel = clayState.quality
        end
    end)

    -- Restore parts
    for inst, data in pairs(clayState.parts) do
        if inst and inst.Parent then
            pcall(function()
                inst.Material = data.Material
                inst.CastShadow = data.CastShadow
                inst.Reflectance = data.Reflectance
                if data.RenderFidelity ~= nil and inst:IsA("MeshPart") then
    ...
    for _, rec in ipairs(clayState.decals) do
        if rec.inst and rec.inst.Parent then
            pcall(function()
                rec.inst.Transparency = rec.prev
            end)
        end
    end
    clayState.decals = {}

    for _, rec in ipairs(clayState.textures) do
        if rec.inst and rec.inst.Parent then
            pcall(function()
                rec.inst.Transparency = rec.prev
            end)
    ...
    for _, rec in ipairs(clayState.surfaceApps) do
        if rec.inst and rec.parent and rec.parent.Parent then
            pcall(function()
                rec.inst.Parent = rec.parent
            end)
        end
    end
    clayState.surfaceApps = {}

    clayState.applied = false
    WindUI:Notify({ Title = "🧱 Clay Mode", Content = "Restored visuals", Duration = 3 })
end

Tabs.PerfTab:Button({
    Title = "🧱 Apply Ultra Clay Mode",
    Desc = "Flatten materials, hide textures, lower draw distance",
    Callback = function()
        applyClayMode()
    end
})
...
Tabs.PerfTab:Button({
    Title = "↩️ Restore Clay Mode",
    Desc = "Restore visuals changed by Clay Mode",
    Callback = function()
        restoreClayMode()
    end
})

-- Kill Effects & Textures (extreme)
local killState = {
    applied = false,
    effects = {}, -- instances with Enabled bool
    lights = {}, -- light instances
    decals = {}, -- {inst, texture, transparency}
    textures = {}, -- {inst, texture, transparency}
    meshTextures = {}, -- {inst, textureId}
    specialMeshTextures = {}, -- {inst, textureId}
    billboards = {}, -- {inst, enabled}
    surfaceGuis = {}, -- {inst, enabled}
    surfaceApps = {}, -- {inst, parent}
}
...
local function applyKillEffects()
    if killState.applied then
        WindUI:Notify({ Title = "🗑️ Effects", Content = "Already killed", Duration = 2 })
        return
    end

    -- Lighting post effects already handled by performance boost, but double enforce
    local Lighting = game:GetService("Lighting")
    if Lighting then
        for _, child in ipairs(Lighting:GetChildren()) do
            if child:IsA("PostEffect") then
                killState.effects[child] = child.Enabled
                safelySet(child, "Enabled", false)
            elseif child:IsA("Atmosphere") then
                killState.effects[child] = child.Density
                safelySet(child, "Density", 0)
            end
        end
    end
    ...
    for _, inst in ipairs(game:GetDescendants()) do
        -- Skip PlayerGui to preserve menus/windows
        if not isInPlayerGui(inst) then
            if inst:IsA("ParticleEmitter") or inst:IsA("Trail") or inst:IsA("Beam") or inst:IsA("Smoke") or inst:IsA("Fire") or inst:IsA("Sparkles") then
                killState.effects[inst] = inst.Enabled
                safelySet(inst, "Enabled", false)
            elseif inst:IsA("Highlight") then
                killState.effects[inst] = inst.Enabled
                safelySet(inst, "Enabled", false)
            elseif inst:IsA("PointLight") or inst:IsA("SpotLight") or inst:IsA("SurfaceLight") then
                killState.lights[inst] = inst.Enabled
                safelySet(inst, "Enabled", false)
            elseif inst:IsA("Decal") then
                table.insert(killState.decals, {inst = inst, texture = inst.Texture, transparency = inst.Transparency})
                pcall(function()
                    inst.Texture = ""
                end)
                safelySet(inst, "Transparency", 1)
            elseif inst:IsA("Texture") then
                table.insert(killState.textures, {inst = inst, texture = inst.Texture, transparency = inst.Transparency})
                pcall(function()
                    inst.Texture = ""
                end)
                safelySet(inst, "Transparency", 1)
            elseif inst:IsA("MeshPart") then
                ...
                if prev ~= nil then
                    table.insert(killState.meshTextures, {inst = inst, textureId = prev})
                end
            elseif inst:IsA("SpecialMesh") then
                ...
                if prev ~= nil then
                    table.insert(killState.specialMeshTextures, {inst = inst, textureId = prev})
                end
            elseif inst:IsA("SurfaceAppearance") then
                table.insert(killState.surfaceApps, {inst = inst, parent = inst.Parent})
                pcall(function()
                    inst.Parent = nil
                end)
            elseif inst:IsA("BillboardGui") then
                killState.billboards[inst] = inst.Enabled
                safelySet(inst, "Enabled", false)
            elseif inst:IsA("SurfaceGui") then
                killState.surfaceGuis[inst] = inst.Enabled
                safelySet(inst, "Enabled", false)
            end
        end
    end

    killState.applied = true
    WindUI:Notify({ Title = "🗑️ Effects", Content = "Disabled effects & textures", Duration = 3 })
end

local function restoreKilledEffects()
    if not killState.applied then
        WindUI:Notify({ Title = "🗑️ Effects", Content = "Nothing to restore", Duration = 2 })
        return
    end

    for inst, prev in pairs(killState.effects) do
        if inst and inst.Parent then
            pcall(function()
                if inst:IsA("Atmosphere") then
                    inst.Density = tonumber(prev) or 0
                else
                    inst.Enabled = prev and true or false
                end
            end)
        end
    end
    ...
    for inst, prev in pairs(killState.lights) do
        if inst and inst.Parent then
            pcall(function()
                inst.Enabled = prev and true or false
            end)
    ...
    for _, rec in ipairs(killState.decals) do
        if rec.inst and rec.inst.Parent then
            pcall(function()
                rec.inst.Texture = rec.texture or rec.inst.Texture
                rec.inst.Transparency = rec.transparency or 0
            end)
        end
    end
    killState.decals = {}

    for _, rec in ipairs(killState.textures) do
        if rec.inst and rec.inst.Parent then
            pcall(function()
                rec.inst.Texture = rec.texture or rec.inst.Texture
                rec.inst.Transparency = rec.transparency or 0
    ...
    for _, rec in ipairs(killState.meshTextures) do
        if rec.inst and rec.inst.Parent then
            pcall(function()
                rec.inst.TextureID = rec.textureId or ""
            end)
        end
    end
    killState.meshTextures = {}

    for _, rec in ipairs(killState.specialMeshTextures) do
        if rec.inst and rec.inst.Parent then
            pcall(function()
                rec.inst.TextureId = rec.textureId or ""
            end)
    ...
    killState.specialMeshTextures = {}

    for _, rec in ipairs(killState.surfaceApps) do
        if rec.inst and rec.parent and rec.parent.Parent then
            pcall(function()
                rec.inst.Parent = rec.parent
            end)
    ...
    killState.surfaceApps = {}

    for inst, prev in pairs(killState.billboards) do
        if inst and inst.Parent then
            pcall(function()
                inst.Enabled = prev and true or false
            end)
    ...
    killState.billboards = {}

    for inst, prev in pairs(killState.surfaceGuis) do
        if inst and inst.Parent then
            pcall(function()
                inst.Enabled = prev and true or false
            end)
    ...
    killState.surfaceGuis = {}

    killState.applied = false
    WindUI:Notify({ Title = "🗑️ Effects", Content = "Restored effects & textures", Duration = 3 })
end

Tabs.PerfTab:Button({
    Title = "🧹 Kill Effects & Textures",
    Desc = "Disable particles, lights, world GUIs, and clear textures",
    Callback = function()
        applyKillEffects()
    end
})
...
Tabs.PerfTab:Button({
    Title = "↩️ Restore Effects & Textures",
    Desc = "Restore disabled effects and textures",
    Callback = function()
        restoreKilledEffects()
    end
})
    ...
    if ok then
        ...
        local function waitForPurchaseConfirmation(preCount, preNet, expectedPrice, timeout)
            ...
            while tick() < deadline do
                if getAvailableEggCountInner() > preCount then return true end
            ...
            if confirmed then
                ...
                if stillMutation then
                    ...
                    WindUI:Notify({ 
                        ...
                        Content = "Bought " .. stillMutation .. " egg for " .. tostring(stillPrice) .. "!", 
                        Duration = 4 
                    })
                end
            else
...
local autoUnlockToggle = Tabs.UnlockTab:Toggle({
...
Tabs.UnlockTab:Button({
    ...
    Callback = function()
        ...
        -- Sort by cost ascending; try cheapest first and re-check funds each time
        table.sort(lockedTiles, function(a, b)
            local ca = parseLockCost(a.cost) or math.huge
            local cb = parseLockCost(b.cost) or math.huge
            return ca < cb
...
local autoDeleteTileFilter = "Both" -- Both | Regular | Ocean
...
local autoDeleteTileDropdown = Tabs.DeleteTab:Dropdown({
    Title = "Tile Filter",
    Desc = "Delete pets on: Both / Regular / Ocean",
            ...
            for _, pet in ipairs(petsFolder:GetChildren()) do
                ...
                if pet:IsA("Model") then
                    ...
                    if petUserId and tonumber(petUserId) == playerUserId then
                        -- Tile filter application -> compute pass flag
                        local passesTileFilter = true
                        if autoDeleteTileFilter ~= "Both" then
                            local petTypeAttr = resolvePetTypeForModel(pet)
                            local isOcean = isOceanPetType(petTypeAttr)
                            if (autoDeleteTileFilter == "Regular" and isOcean) or (autoDeleteTileFilter == "Ocean" and (not isOcean)) then
                                passesTileFilter = false
                            end
                        end
                        if passesTileFilter then
                            -- Check pet's speed
